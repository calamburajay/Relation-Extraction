...
class model:
    ...
    def broadcasting(self, left, right):
        ...
        left = tf.transpose(a=left, perm=[1, 0, 2])
        left = tf.expand_dims(left, 3)

        right = tf.transpose(a=right, perm=[0, 2, 1])
        right = tf.expand_dims(right, 0)

        B = left + right
        B = tf.transpose(a=B, perm=[1, 0, 3, 2])

        return B

    def getNerScores(self, gru_out, n_types=1, dropout_keep_in_prob=1):
        import tensorflow as tf


        u_a = tf.compat.v1.get_variable("u_typ", [self.config.hidden_size_gru * 2, self.config.hidden_size_n1])  # [128 32]
        ...
        mul = tf.einsum('aij,jk->aik', gru_out, u_a)  # [16 348 64] * #[64 32] = [16 348 32]
        ...
        if self.config.use_dropout==True:
            output = tf.nn.dropout(output, rate=1 - (dropout_keep_in_prob))
    ...
    def getHeadSelectionScores(self, gru_out,dropout_keep_in_prob=1):
        import tensorflow as tf

        u_a = tf.compat.v1.get_variable("u_a", [(self.config.hidden_size_gru * 2) + self.config.label_embeddings_size, self.config.hidden_size_n1])  # [128 32]
        w_a = tf.compat.v1.get_variable("w_a", [(self.config.hidden_size_gru * 2) + self.config.label_embeddings_size, self.config.hidden_size_n1])  # [128 32]
        ...
        left = tf.einsum('aij,jk->aik', gru_out, u_a)  # [16 348 64] * #[64 32] = [16 348 32]
        right = tf.einsum('aij,jk->aik', gru_out, w_a)  # [16 348 64] * #[64 32] = [16 348 32]
        ...
        g = tf.reshape(g, [tf.shape(input=g)[0], tf.shape(input=g)[1], tf.shape(input=g)[2] * len(self.config.dataset_set_relations)])
    ...
    def computeLoss(self,input_rnn, dropout_embedding_keep,dropout_gru_keep,dropout_gru_output_keep,
        ...
        with tf.compat.v1.variable_scope("loss_computation", reuse=reuse):
            ...
            if self.config.use_dropout:
                    input_rnn = tf.nn.dropout(input_rnn, rate=1 - (dropout_embedding_keep))
            ...
            for i in range(self.config.num_gru_layers):
                if self.config.use_dropout and i>0:
                    input_rnn = tf.nn.dropout(input_rnn, rate=1 - (dropout_gru_keep))
                    #input_rnn = tf.Print(input_rnn, [dropout_gru_keep], 'gru:  ', summarize=1000)

                gru_fw_cell = tf.keras.layers.GRUCell(self.config.hidden_size_gru)
                # Backward direction cell
                gru_bw_cell = tf.keras.layers.GRUCell(self.config.hidden_size_gru)

                gru_out, _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(
                    cell_fw=gru_fw_cell,
                    cell_bw=gru_bw_cell,
                    inputs=input_rnn,
                    sequence_length=seqlen,
                    dtype=tf.float32, scope='Bigru' + str(i))

                input_rnn = tf.concat(gru_out, 2)

                gru_output = input_rnn

            if self.config.use_dropout:
                gru_output = tf.nn.dropout(gru_output, rate=1 - (dropout_gru_output_keep))


            mask = tf.sequence_mask(seqlen, dtype=tf.float32)

            ner_input = gru_output
            ...
            if self.config.ner_loss == "crf":
                ...
                predNers, viterbi_score = tfa.text.crf_decode(
                ...
                predNers = tf.cast(tf.argmax(nerScores, 2), tf.int32)


            if self.config.label_embeddings_size > 0:

                labels = tf.cond(pred=is_train > 0, true_fn=lambda: ners_ids, false_fn=lambda: predNers)


                label_embeddings = tf.nn.embedding_lookup(params=label_matrix, ids=labels)
                rel_input = tf.concat([gru_output, label_embeddings], axis=2)

            else:

                rel_input = gru_output
    ...
    def run(self):
        ...
        dropout_gru_keep = tf.compat.v1.placeholder(tf.float32, name="dropout_gru_keep")
        dropout_gru_output_keep = tf.compat.v1.placeholder(tf.float32, name="dropout_gru_output_keep")
        ...
        embedding_matrix = tf.compat.v1.get_variable(name="embeddings", shape=self.emb_mtx.shape,
                                           initializer=tf.compat.v1.constant_initializer(self.emb_mtx), trainable=False)
        ...
        char_embeddings = tf.nn.embedding_lookup(params=K, ids=char_ids)

        # 2. put the time dimension on axis=1 for dynamic_rnn
        s = tf.shape(input=char_embeddings)  # store old shape
        ...
        # 3. bi gru on chars
        cell_fw = tf.keras.layers.GRUCell(char_hidden_size)
        cell_bw = tf.keras.layers.GRUCell(char_hidden_size)

# _, ((_, output_fw), (_, output_bw))
        ((output_fw,output_bw),_) = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=cell_fw, cell_bw=cell_bw,
        ...
        word_embeddings = tf.nn.embedding_lookup(params=embedding_matrix, ids=embedding_ids)
        ...
        lossNER, lossREL, predicted_entity_tags_ids, predictedRel, rel_scores = self.computeLoss(input_rnn,
                                                                                ...
                                                                                dropout_gru_keep,
                                                                                dropout_gru_output_keep, seqlen,
                                                                                dropout_fcl_ner_keep,
                                                                                entity_tags_ids, dropout_fcl_rel_keep,
                                                                                is_train,
                                                                                scoring_matrix_gold,reuse=False)

        obj = tf.reduce_sum(input_tensor=lossNER) + tf.reduce_sum(input_tensor=lossREL)
        #perturb the inputs
        raw_perturb = tf.gradients(ys=obj, xs=embeddings_input)[0]  # [batch, L, dim]
        normalized_per=tf.nn.l2_normalize(raw_perturb, axis=[1, 2])
        perturb =self.config.alpha*tf.sqrt(tf.cast(tf.shape(input=input_rnn)[2], tf.float32)) * tf.stop_gradient(normalized_per)
        ...
        if self.config.use_adversarial==True:
            ...
            obj+=tf.reduce_sum(input_tensor=lossNER_per)+tf.reduce_sum(input_tensor=lossREL_per)
        ...
        m['dropout_gru']=dropout_gru_keep
        m['dropout_gru_output']=dropout_gru_output_keep
