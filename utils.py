...
def printParameters(config):
    ...
    t.add_row(['Dropout gru ', config.dropout_gru])
    t.add_row(['Dropout gru output ', config.dropout_gru_output])
    t.add_row(['Dropout fcl ner ', config.dropout_fcl_ner])
    t.add_row(['Dropout fcl rel ', config.dropout_fcl_rel])
    t.add_row(['Hidden gru size ', config.hidden_size_gru])
    t.add_row(['gru layers ', config.num_gru_layers])
...
def generator(data, m,config,train=False):
    ...
    dropout_gru_keep = m['dropout_gru']
    dropout_gru_output_keep = m['dropout_gru_output']
    ...
    if config.use_dropout == True and train==True:
        ...
        dropout_gru_prob = config.dropout_gru
        dropout_gru_output_prob = config.dropout_gru_output
    ...
    for bIdx in range(len(batches_embeddingIds)):  # 131
        ...
        yield {dropout_embedding_keep:dropout_embedding_prob,dropout_gru_keep:dropout_gru_prob,dropout_gru_output_keep:dropout_gru_output_prob,
